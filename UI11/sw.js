...
const CACHE_VERSION = "1.33.6-original-location-pin";
...
const CACHE_NAME = `barrierfree-map-v${CACHE_VERSION}-stepby-ui11`;
// 画像（プロフィール画像など）はバージョンが変わっても保持し続けるための専用キャッシュ。
// CACHE_NAMEと違いタイムスタンプを含めず、activateハンドラの古いキャッシュ削除フィルタにも引っかからない名前にする。
const IMAGE_CACHE_NAME = "barrierfree-map-images-ui11-v1";
  ...
  event.waitUntil(
    caches.keys().then((keys) => {
      ...
      const deletePromises = keys
        .filter((key) => key.includes("-stepby-ui11-") && key !== CACHE_NAME)
        .map((key) => {
